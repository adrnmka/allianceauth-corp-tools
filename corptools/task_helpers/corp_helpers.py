...
def update_corp_wallet_journal(corp_id, wallet_division, full_update=False):
    ...
    _current_eve_ids = set(list(EveName.objects.all().values_list("eve_id", flat=True)))
...
def update_corporation_transactions(corp_id, wallet_division, full_update=False):
    ...
    logger.debug(
        "Updating market transactions for: {} (Div: {})".format(
            audit_corp.corporation.corporation_name, division
        )
    )
    ...
    try:
        ...
        journal_items = etag_results(journal_items_ob, token, force_refresh=full_update)
        ...
        _current_journal = CorporationWalletJournalEntry.objects.filter(
            ...
            # Max items from ESI
            reason__exact="",
        ).values_list("context_id", flat=True)[:2500]
...
def update_corporation_pocos(corp_id, full_update=False):
    ...
    logger.debug(
        "Updating Pocos for: {}".format(audit_corp.corporation.corporation_name)
    )
    ...
    try:
        ...
        poco_data = etag_results(poco_ob, token, force_refresh=full_update)
        ...
        for id_chunk in providers.esi.chunk_ids(_all_ids):
            try:
                ...
                logger.error(
                    f"OSError when fetching asset names for corp {corp_id}: {e}"
                )
                return f"OSError during asset name resolution for corp {corp_id}"
            except Exception as e:
                logger.error(
                    f"Unexpected error during asset name resolution for corp {corp_id}: {e}"
                )
                return f"Error during asset name resolution for corp {corp_id}"
        ...
        for system_id in _all_system_ids:
            try:
                ...
                logger.error(f"OSError when fetching system info for {system_id}: {e}")
                continue
            except Exception as e:
                logger.error(
                    f"Unexpected error when fetching system info for {system_id}: {e}"
                )
                continue
        ...
        logger.info(
            "CT: No New Poco data for: {}".format(
...
def update_corp_wallet_division(corp_id, full_update=False):  # pagnated results
    ...
    divisions = providers.esi.client.Wallet.get_corporations_corporation_id_wallets(
        corporation_id=audit_corp.corporation.corporation_id,
        token=token.valid_access_token(),
    ).result()
    ...
    for division in divisions:
        ...
        if _division_item:
            update_corp_wallet_journal(
                corp_id, division.get("division"), full_update=full_update
            )  # inline not async
...
def update_corp_structures(corp_id, force_refresh=False):  # pagnated results
    ...
    def _structures_db_update(_corporation, _structure, _name):
        ...
        _structure_ob, _created = Structure.objects.update_or_create(
            ...
            defaults={
                ...
                "next_reinforce_weekday": _structure.get(
                    ...
                    if db_service.exists():
                        if db_service.count() == 1:
                            ...
                            StructureService.objects.filter(
                                structure=_structure_ob, name=service["name"]
                            ).delete()
    ...
    _corporation = CorporationAudit.objects.get(corporation__corporation_id=corp_id)

    structure_ids = []
    operation = (
        providers.esi.client.Corporation.get_corporations_corporation_id_structures(
            corporation_id=_corporation.corporation.corporation_id
        )
    )
    try:
    ...
    for structure in structures:
        ...
        if structure.get("name"):
            ...
            try:
                structure_info = fetch_location_name(
                    structure.get("structure_id"), "solar_system", token.character_id
                )
        ...
        try:
            structure_ob, created = _structures_db_update(_corporation, structure, name)

        except MultipleObjectsReturned:
            id_of_first = (
                Structure.objects.filter(structure_id=structure.get("structure_id"))
                .order_by("id")[0]
                .id
            )
            Structure.objects.filter(
                structure_id=structure.get("structure_id")
            ).exclude(id=id_of_first).delete()
            structure_ob, created = _structures_db_update(_corporation, structure, name)
...
def update_character_logins_from_corp(corp_id):
    audit_corp = CorporationAudit.objects.get(corporation__corporation_id=corp_id)
    logger.debug("Updating Logins for: {}".format(audit_corp.corporation))
    ...
    try:
        ...
        logger.info("CT: No New Logins for: {}".format(audit_corp.corporation))
        pass

    all_chars = CharacterAudit.objects.filter(character__corporation_id=corp_id)
...
def update_corp_assets(corp_id):
    audit_corp = CorporationAudit.objects.get(corporation__corporation_id=corp_id)
    logger.debug("Updating Assets for: {}".format(audit_corp.corporation))
    ...
    try:
        ...
        for item in assets:
            ...
            asset_item = CorpAsset(
                corporation=audit_corp,
                blueprint_copy=item.get("is_blueprint_copy"),
                singleton=item.get("is_singleton"),
                item_id=item.get("item_id"),
                location_flag=sanitize_location_flag(item.get("location_flag")),
        ...
        delete_query = CorpAsset.objects.filter(corporation=audit_corp)  # Flush Assets
        ...
        logger.info("CT: No New assets for: {}".format(audit_corp.corporation))
        pass
...
def update_corp_asset_names(corp_id):
    ...
    for subset in chunks(asset_list, 100):
        ...
        assets_names = (
            providers.esi.client.Assets.post_corporations_corporation_id_assets_names(
                corporation_id=corp_id,
...
def run_ozone_levels(self, corp_id):
    ...
    for structure in _structures:
        _quantity = CorpAsset.objects.filter(
            corporation=_corporation, location_id=structure.structure_id, type_id=16273
        ).aggregate(ozone=Sum("quantity"))["ozone"]
        _used = 0

        try:
            last_ozone = (
                BridgeOzoneLevel.objects.filter(station_id=structure.structure_id)
                .order_by("-date")[:1][0]
                .quantity
            )
...
def build_managed_asset_locations(self, corp_id):
    _corporation = CorporationAudit.objects.get(corporation__corporation_id=corp_id)
    logger.debug(
        "Updating Corporate Hangar Locations for: %s" % (_corporation.corporation)
    )
    ...
    existing_locations = set(
        list(
            EveLocation.objects.filter(
                managed=True, managed_corp=_corporation
    ...
    folders = CorpAsset.objects.filter(
        corporation=_corporation, location_flag="OfficeFolder"
    ...
    for i in folders:
        ...
        for j in range(1, 8):
            ...
            CorpAsset.objects.filter(
                corporation=_corporation, location_id=i.item_id, location_flag=sag
            ).update(location_name_id=id)
    # get Containers in office folders and assign the names

    containers = (
        CorpAsset.objects.filter(
            Q(type_name__group_id__in=grp_ids)
            | Q(type_name__group__category_id__in=cat_ids)
        )
        .filter(
            corporation=_corporation,
            singleton=True,
            location_id__in=folders.values_list("item_id", flat=True),
        )
    ...
    EveLocation.objects.filter(managed=True, managed_corp=_corporation).exclude(
        location_id__in=current + hangar_ids
    ).delete()
...
def build_jb_network(self):
    structures = (
        Structure.objects.filter(type_id=35841)
    ...
    for s in structures:
        ...
        output[matches[0]]["end"] = {"system": _exit, "name": _exit.name}
...
def update_corporate_contracts(corp_id, force_refresh=False):
    _corporation = CorporationAudit.objects.get(corporation__corporation_id=corp_id)
    logger.debug(
        "updating corporate contracts for: %s"
    ...
    try:
        contracts_op = (
            providers.esi.client.Contracts.get_corporations_corporation_id_contracts(
                corporation_id=corp_id
            )
        )

        contracts = etag_results(contracts_op, token, force_refresh=force_refresh)

        contract_models_new = []
        contract_models_old = []
        eve_names = set()
        contract_ids = list(
            CorporateContract.objects.filter(corporation=_corporation).values_list(
                "contract_id", flat=True
            )
        )
        for c in contracts:  # update labels
            _contract_item = CorporateContract(
                id=CorporateContract.build_pk(_corporation.id, c.get("contract_id")),
                corporation=_corporation,
    ...
    return (
        "CT: Completed corporate contracts for: %s"
        % str(_corporation.corporation.corporation_name),
        new_contract_ids,
    )
...
def update_corporate_contract_items(self, corp_id, contract_id, force_refresh=False):
    ...
    contract = CorporateContract.objects.get(
        corporation=_corporation, contract_id=contract_id
    )
    ...
    if contract.status != "deleted":
        ...
        try:
            contracts_op = providers.esi.client.Contracts.get_corporations_corporation_id_contracts_contract_id_items(
                corporation_id=corp_id, contract_id=contract_id
            )
            ...
            try:
                ...
                if e.status_code == 404:
                    logger.warning(
                        "CT: Contract items {} ({}) NOT FOUND ERROR".format(
                            str(_corporation.corporation.corporation_name),
                            str(contract_id),
                        )
                    )
...
def fetch_coordiantes(self, corp_id):
    ...
    logger.info(f"CT: Starting Coords for {_corporation.corporation.corporation_name}")
    ...
    locations = (
        providers.esi.client.Assets.post_corporations_corporation_id_assets_locations(
            corporation_id=_corporation.corporation.corporation_id,
            item_ids=list(assets.values_list("item_id", flat=True)),
    ...
    for a in assets:
        if a.item_id in new_coords:
            ...
            logger.info(f"CT: COORD - {a.type_name.name} {new_coords[a.item_id]}")
...
def fetch_starbases(corp_id, force_refresh=True):  # Set true as we have bad data
    ...
    starbases_ob = (
        providers.esi.client.Corporation.get_corporations_corporation_id_starbases(
            corporation_id=_corporation.corporation.corporation_id,
        )
    )
    try:
        starbases = etag_results(starbases_ob, _token, force_refresh=force_refresh)
        ...
        for sb in starbases:
            ...
            Starbase.objects.update_or_create(
                ...
                defaults={
                    ...
                    "allow_corporation_members": starbase.get(
                        "allow_corporation_members"
                    ),
                    "anchor": starbase.get("anchor"),
                    "attack_if_at_war": starbase.get("attack_if_at_war"),
                    "attack_if_other_security_status_dropping": starbase.get(
                        "attack_if_other_security_status_dropping"
                    ),
                    "attack_security_status_threshold": starbase.get(
                        "attack_security_status_threshold"
                    ),
                    "attack_standing_threshold": starbase.get(
        ...
        Starbase.objects.filter(
            corporation=_corporation,
        ).exclude(starbase_id__in=ids).delete()
